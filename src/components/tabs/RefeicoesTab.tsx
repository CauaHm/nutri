...
import { CARD, PURP, SUB, BORDER, sInp, sLbl, sBtn } from "@/lib/theme";
...
interface FormState {
  ...
  emoji: string;
  horario: string;
  kcalAlvo: string | number;
  itens: string;
}

const EMPTY: FormState = { nome: "", emoji: "🍽️", horario: "", kcalAlvo: "", itens: "" };

export default function RefeicoesTab({ data }: TabProps) {
  const { user, refeicoesCfg, saveRefeicoesCfg } = data;
  const [editId, setEditId] = useState<string | null>(null);
  const [editVal, setEditVal] = useState<FormState>(EMPTY);
  const [addForm, setAddForm] = useState(false);
  const [newRef, setNewRef] = useState<FormState>(EMPTY);

  if (!refeicoesCfg) return null;

  const startEdit = (r: RefeicaoConfig) => {
    setEditId(r.id);
  ...
  const delRef = async (id: string) => saveRefeicoesCfg(refeicoesCfg.filter((r) => r.id !== id));
  const addRef = async () => {
    if (!newRef.nome) return;
    const r: RefeicaoConfig = {
