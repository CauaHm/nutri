...
import type { Receita } from "@/lib/defaults";
...
const EMPTY: FormState = { nome: "", tempo: "", custo: "", ing: "", passos: "", dica: "" };

export default function ReceitasTab({ data }: TabProps) {
  const { user, receitas, saveReceitas } = data;
  const [aberta, setAberta] = useState<string | null>(null);
  const [editId, setEditId] = useState<string | null>(null);
  const [editVal, setEditVal] = useState<FormState>(EMPTY);
  const [addForm, setAddForm] = useState(false);
  const [newRec, setNewRec] = useState<FormState>(EMPTY);
  ...
  const delRec = async (id: string) => saveReceitas(receitas.filter((r) => r.id !== id));
  const addRec = async () => {
