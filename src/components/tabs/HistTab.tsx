...
function groupByWeek<T>(entries: T[], getDate: (e: T) => string): { wk: string; dias: T[] }[] {
...
export default function HistTab({ data }: TabProps) {
  const { user, mealLog, waters, delWater } = data;
  const [semAberta, setSemAberta] = useState<Record<string, boolean>>({});
