...
import {
  ...
  snapshotsPorSemana, compararSemanas, type Sexo,
} from "@/lib/bodycomp";
...
const CAMPOS_LABEL: Record<string, string> = { peso: "Peso", cintura: "Cintura", pescoco: "Pescoço", ...Object.fromEntries(MEDIDAS_LABEL.map((m) => [m.key, m.label])) };
const CAMPOS_UNIDADE: Record<string, string> = { peso: "kg" };
...
const NOVA_MED_VAZIA: MedicaoForm = { date: todayStr(), peso: "", cintura: "", pescoco: "", quadril: "", gorduraMedida: "", ombros: "", bunda: "", braco: "", coxa: "", panturrilha: "" };
...
export default function ComposicaoScreen({ data, nav, auth }: ScreenProps) {
  ...
  const [novaMed, setNovaMed] = useState<MedicaoForm>(NOVA_MED_VAZIA);
  const [editMetas, setEditMetas] = useState(false);
  const [metasTemp, setMetasTemp] = useState<Record<string, any> | null>(null);
  const [objTemp, setObjTemp] = useState<ObjTemp | null>(null);
  ...
  const macros = preview ? calcMacros(peso, preview.caloriasDiarias, Number(objTemp.proteinaGkg), Number(objTemp.gorduraGkg)) : null;
          ...
          <div style={{ padding: 14 }}>
            {!peso ? (
              ...
              <>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {OBJETIVOS.map((o) => (
                    <button key={o.key} onClick={() => setObjTemp((p) => (p ? { ...p, objetivo: o.key } : p))} style={{ flex: 1, padding: "8px 4px", background: objTemp.objetivo === o.key ? `${GRN}30` : "#ffffff08", border: `1px solid ${objTemp.objetivo === o.key ? GRN : BORDER}`, borderRadius: 8, color: objTemp.objetivo === o.key ? GRN : SUB, fontSize: 10.5, cursor: "pointer", fontWeight: objTemp.objetivo === o.key ? 700 : 400 }}>{o.label}</button>
                  ))}
                </div>

                {objTemp.objetivo !== "manter" && (
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <div><label style={sLbl}>Peso meta (kg)</label><input style={sInp} type="number" step="0.5" value={objTemp.pesoMeta} onChange={(e) => setObjTemp((p) => (p ? { ...p, pesoMeta: e.target.value } : p))} /></div>
                    <div><label style={sLbl}>Prazo (semanas)</label><input style={sInp} type="number" value={objTemp.prazoSemanas} onChange={(e) => setObjTemp((p) => (p ? { ...p, prazoSemanas: e.target.value } : p))} /></div>
                  </div>
                )}
        ...
        {bc.historico.length === 0 ? (
          ...
          <>
            <div style={{ ...sCard, padding: 14, marginBottom: 10 }}>
              <BodyCompChart title="Peso" color={CYAN} unit="kg" points={chartPontos.filter((p) => p.peso !== null).map((p) => ({ x: p.date, y: p.peso as number, label: p.label }))} />
            </div>
            <div style={{ ...sCard, padding: 14, marginBottom: 10 }}>
              <BodyCompChart title="% de Gordura" color={PINK} unit="%" points={chartPontos.filter((p) => p.gordura !== null).map((p) => ({ x: p.date, y: Math.round((p.gordura as number) * 10) / 10, label: p.label }))} />
            </div>
