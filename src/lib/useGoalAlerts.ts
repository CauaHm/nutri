...
export function useGoalAlerts(userId: string | null | undefined, metrics: GoalMetric[]) {
  const [alert, setAlert] = useState<GoalAlert | null>(null);
  const seenRef = useRef<Set<string>>(new Set());
  const loadedForRef = useRef<string | null | undefined>(null);
