...
export default function TreinoDiaScreen({ data, nav, params }) {
  ...
  const [diaMetaTemp, setDiaMetaTemp] = useState(null);
        ...
        {editDiaMeta && (
          <div style={{ ...sCard, padding: 14, marginBottom: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 8 }}>
              <div><label style={sLbl}>Tipo</label><input style={sInp} value={diaMetaTemp.tag} onChange={(e) => setDiaMetaTemp((p) => ({ ...p, tag: e.target.value }))} /></div>
              <div><label style={sLbl}>Emoji</label><input style={sInp} value={diaMetaTemp.emoji} onChange={(e) => setDiaMetaTemp((p) => ({ ...p, emoji: e.target.value }))} /></div>
            </div>
            <label style={sLbl}>Cardio</label><input style={sInp} value={diaMetaTemp.cardio} onChange={(e) => setDiaMetaTemp((p) => ({ ...p, cardio: e.target.value }))} placeholder="ex: 40 min esteira" />
            <label style={sLbl}>Informação extra (opcional)</label><input style={sInp} value={diaMetaTemp.info} onChange={(e) => setDiaMetaTemp((p) => ({ ...p, info: e.target.value }))} />
