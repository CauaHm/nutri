...
import { calcGorduraNavy, calcMassas, calcTMB, classificarGordura } from "@/lib/bodycomp";
import { CARD2, SUB, TEXT, sCard } from "@/lib/theme";
import CompareChart from "@/components/CompareChart";

const num = (v) => (v === "" || v === null || v === undefined ? null : Number(v));
const fmt = (v, casas = 1) => (v === null || v === undefined || Number.isNaN(v) ? "—" : v.toFixed(casas));

function calcularAtual(pessoa, historico) {
  const latest = historico[0];
  if (!latest) return null;
  const sexo = pessoa.sexo, altura = num(pessoa.altura), idade = num(pessoa.idade);
...
function StatRow({ label, a, b, corA, corB, unidade = "" }) {
  return (
...
export default function ComparativoScreen({ data, nav }) {
  const { user, outroUser, outroId, temParceiro } = data;
  const [historicoParceiro, setHistoricoParceiro] = useState(null);
  const [historicoMeu, setHistoricoMeu] = useState(null);
  ...
  const pesoSerieA = [...historicoMeu].sort((a, b) => a.id - b.id).filter((h) => h.peso).map((h) => ({ date: h.date, y: num(h.peso) }));
  const pesoSerieB = [...historicoParceiro].sort((a, b) => a.id - b.id).filter((h) => h.peso).map((h) => ({ date: h.date, y: num(h.peso) }));
  pesoSerieA.nome = user.nome.split(" ")[0]; pesoSerieA.color = user.cor;
  pesoSerieB.nome = outroUser.nome.split(" ")[0]; pesoSerieB.color = outroUser.cor;

  const gordSerieFrom = (hist, pessoa) => [...hist].sort((a, b) => a.id - b.id).map((h) => {
    const g = h.gorduraMedida !== "" && h.gorduraMedida != null ? num(h.gorduraMedida)
      : calcGorduraNavy({ sexo: pessoa.sexo, altura: num(pessoa.altura), cintura: num(h.cintura), pescoco: num(h.pescoco), quadril: num(h.quadril) });
    return g !== null ? { date: h.date, y: Math.round(g * 10) / 10 } : null;
  }).filter(Boolean);
  const gordSerieA = gordSerieFrom(historicoMeu, user);
  const gordSerieB = gordSerieFrom(historicoParceiro, outroUser);
  gordSerieA.nome = user.nome.split(" ")[0]; gordSerieA.color = user.cor;
  gordSerieB.nome = outroUser.nome.split(" ")[0]; gordSerieB.color = outroUser.cor;
