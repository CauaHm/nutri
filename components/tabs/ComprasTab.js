...
import { CARD, GRN, PURP, PINK, SUB, BORDER, corDeTexto, sInp, sLbl, sBtn } from "@/lib/theme";

const EMPTY = { item: "", qtd: "", preco: "", cat: "" };

export default function ComprasTab({ data }) {
  ...
  const addItem = async () => {
    ...
    const c = { id: `c${Date.now()}`, item: newItem.item, qtd: newItem.qtd, preco: Number(newItem.preco) || 0, cat: newItem.cat || "Outro" };
    await saveCompras([...compras, c]);
    setAddForm(false);
    setNewItem(EMPTY);
  };
  const delItem = async (id) => saveCompras(compras.filter((c) => c.id !== id));
  const startEdit = (c) => { setEditId(c.id); setEditVal({ ...c, preco: String(c.preco) }); };
