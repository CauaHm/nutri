...
export async function POST(req) {
  ...
  const { imageBase64, imageMediaType, descricao } = await req.json();
  ...
  try {
    ...
    return NextResponse.json({ error: "api_error", message: err?.message || String(err) }, { status: 502 });
  }
  ...
  return NextResponse.json({ alimento: parsed });
}
